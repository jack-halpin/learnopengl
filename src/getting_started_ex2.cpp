...
/* Note: To make two triangles we have to add
extra vertices to and the in the glDrawArrays
method we have to specify that we want to draw
6 vertices instead of 3

For exercise two we need to create two seperate VAO each
with different vector attributes. The original solution
says to use two seperate VBO but I've used the same one in this
case and adjusted the offsets in glVertexAttribPointer for the
second VAO. */

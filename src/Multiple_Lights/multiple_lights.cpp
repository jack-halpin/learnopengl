...
int main()
{
	...
	// Generate a VBO in order to store the data on the graphics card
	unsigned int VBO;
	glGenBuffers(1, &VBO);

	Shader objectShader(createShaderPath("vertex_shader_multiple_lights.vs").c_str(), createShaderPath("fragment_shader_multiple_lights.fs").c_str());
	Shader lightShader(createShaderPath("vertex_shader_multiple_lights.vs").c_str(), createShaderPath("fragment_shader_light_lighting.fs").c_str());

	// Create Texture value to use as diffuse map
	...
	// We are just using the cube vertices data for now so we can just use the same VBO
	// The data was already previously entered above
	glBindBuffer(GL_ARRAY_BUFFER, VBO);
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 8 * sizeof(float), (void*)0);
	glEnableVertexAttribArray(0);
